...
/**
 * Консьюмер для билетов джекпот лотереи
 */
class ConsumeJackpotTicketsCommand extends ConsumeBase
{
    protected static string $defaultName = 'consume-tickets:jackpot';
    protected static string $defaultDescription = 'Консьюмер билетов jackpot_tickets_v1';
    ...
    {
        $this->topic = config('kafka.jackpot_tickets_topic');
    }

    protected function getLogChannel(): string
    {
        return 'command_consume_jackpot_tickets';
    }
    ...
    public function consumerLogic(): Closure
    {
        return function (array $body, array $headers, $logger) {
            ...
            $this->ticketImportService->createPartitionIfNotExists('jackpot_tickets', $lotteryId);
            ...
            foreach ($tickets as $ticketNumber) {
                ...
                $this->ticketImportService->saveTicketToPartition('jackpot_tickets', $ticketData);
                $savedCount++;
            }

            $logger->info('Билеты jackpot сохранены', [
                'lottery_id' => $lotteryId,
