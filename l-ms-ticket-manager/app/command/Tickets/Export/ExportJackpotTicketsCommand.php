...
class ExportJackpotTicketsCommand extends Command
{
    protected static $defaultName = 'export_tickets:jackpot';
    protected static $defaultDescription = 'Экспорт билетов джекпот лотереи';
    ...
    {
        parent::__construct();
        $this->logger = Log::channel('command_export_jackpot_tickets');
        $this->ticketExportService = new TicketExportService('kafka.jackpot_tickets_topic', $this->logger);
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!$lotteryId) {
            $this->logger->info('Получение всех активных джекпот лотерей');
            $lotteryIds = $this->ticketExportService->getActiveLotteryIdsByType(LotteryTypeEnum::JACKPOT);
            if (empty($lotteryIds)) {
                $this->logger->info('Не найдено активных лотерей типа "' . LotteryTypeEnum::JACKPOT . '"');
                $executionTime = round((microtime(true) - $startTime) * 1000, 2);
                $output->writeln("Команда завершена: Нет активных джекпот лотерей, время: {$executionTime}ms");
        ...
        if (!empty($errors)) {
            ...
            $output->writeln("Команда завершена: Экспорт jackpot - обработано {$totalExported} из " . count($lotteryIds) . " лотерей, время: {$executionTime}ms");
        ...
        $output->writeln("Команда завершена: Экспорт билетов jackpot для {$totalExported} лотерей завершён, время: {$executionTime}ms");
