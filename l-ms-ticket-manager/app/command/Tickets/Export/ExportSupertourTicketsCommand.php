...
class ExportSupertourTicketsCommand extends Command
{
    protected static $defaultName = 'export_tickets:supertour';
    protected static $defaultDescription = 'Экспорт билетов супертур лотереи';
    ...
    {
        parent::__construct();
        $this->logger = Log::channel('command_export_supertour_tickets');
        $this->ticketExportService = new TicketExportService('kafka.supertour_tickets_topic', $this->logger);
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!$lotteryId) {
            $this->logger->info('Получение всех активных супертур лотерей');
            $lotteryIds = $this->ticketExportService->getActiveLotteryIdsByType(LotteryTypeEnum::SUPERTOUR);
            if (empty($lotteryIds)) {
                $this->logger->info('Не найдено активных лотерей типа "' . LotteryTypeEnum::SUPERTOUR . '"');
                $executionTime = round((microtime(true) - $startTime) * 1000, 2);
                $output->writeln("Команда завершена: Нет активных супертур лотерей, время: {$executionTime}ms");
        ...
        if (!empty($errors)) {
            ...
            $output->writeln("Команда завершена: Экспорт supertour - обработано {$totalExported} из " . count($lotteryIds) . " лотерей, время: {$executionTime}ms");
        ...
        $output->writeln("Команда завершена: Экспорт билетов supertour для {$totalExported} лотерей завершён, время: {$executionTime}ms");
